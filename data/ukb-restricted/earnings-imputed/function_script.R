...
# IMPRTANT : The data you will provide to the function must contain the follwoing variables with the same name and class specified 
    # "SOC" (numeric or factor): 4-digit standaridzed occupation code (2000)
...
for (i in y){
  ...
  D[year==i, soc_mean_hourly:=ifelse(male == 1, ONS_temp[rown, "mean_male"], ONS_temp[rown, "mean_female"])]
  
  #if 4-digit info missing
  rown <- match(substr(soc, 1, 3), ONS_temp$code) 
  D[year==i,  soc_mean_hourly:=ifelse(!is.na(soc) & is.na(soc_mean_hourly), 
                                                ifelse(male == 1, ONS_temp[rown, "mean_male"], ONS_temp[rown, "mean_female"]),
                                                    soc_mean_hourly) ]
  ...
  rown <- match(soc, ONS_temp$code) 
  D[year==i, soc_median_hourly:=ifelse(male == 1, ONS_temp[rown, "median_male"], ONS_temp[rown, "median_female"])]
  
  #if 4-digit info missing
  rown <- match(substr(soc, 1, 3), ONS_temp$code) 
  D[year==i,  soc_median_hourly:=ifelse(!is.na(soc) & is.na(soc_median_hourly), 
                                                ifelse(male == 1, ONS_temp[rown, "median_male"], ONS_temp[rown, "median_female"]),
                                                    soc_median_hourly) ]
  # if 3-digit info also missing
  rown <- match(substr(soc, 1, 2), ONS_temp$code) 
  D[year==i,  soc_median_hourly:=ifelse(!is.na(soc) & is.na(soc_median_hourly), 
  ...
  #if 4-digit info missing
  rown <- match(substr(soc, 1, 3), ONS_temp$code) 
  D[year==i, soc_mean_hourly_all:= ifelse(!is.na(soc) & is.na(soc_mean_hourly_all), ONS_temp[rown, "mean_all"], soc_mean_hourly_all) ]
  ...
  #if 4-digit info missing
  rown <- match(substr(soc, 1, 3), ONS_temp$code) 
  D[year==i, soc_median_hourly_all:= ifelse(!is.na(soc) & is.na(soc_median_hourly_all), ONS_temp[rown, "median_all"], soc_median_hourly_all) ]
  ...
  D[year==i, soc_mean_hourly:=ifelse(!is.na(soc) & is.na(soc_mean_hourly), soc_mean_hourly_all, soc_mean_hourly)]
  D[year==i, soc_median_hourly:=ifelse(!is.na(soc) & is.na(soc_median_hourly), soc_median_hourly_all, soc_median_hourly)]
   
  #convert to 2015 currency  
  D[year==i, soc_mean_hourly:=soc_mean_hourly  / cpi[i-2001, 2] ]  ## this syntax works for data.frame only (cpi)
