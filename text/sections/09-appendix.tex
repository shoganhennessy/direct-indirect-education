...
\begin{table}[!h]
    ...
    \begin{tabular}{l c c c}
        ...
        \\[-1.8ex]\hline \hline \\[-1.8ex] 
        \input{sections/tables/ukb-relations-count.tex}
        \\[-1.8ex]\hline \\[-1.8ex]
    \end{tabular}
    \vspace{-0.25cm}
...
and indirect effects, evaluated at $z = Z_i^\text{\textcolor{childColour}{Child}}$.
...
Note that the economics literature has argued that null estimates for education returns in the MLSA design are a result of the MLSA rise inducing a low-quality margin of education \citep{clark2013effect}, so while age leaving education variables are more significant for this instrument they may be economically relevant.
...
This gives occupation coded hourly income, and annual wage calculated from hours worked as outcome measures in the MLSA rise, affected by the MSLA rise
...
\cite{barcellos2021effect} analyse this same RDD with UKB data, and a hard coded RDD specification; this Appendix subsection returns to this RDD and finds that returns to education estimates among the UKB population (or any subsample) are not robust to RDD best practices.
...
These results contrast with those in \citep{barcellos2021effect}, which does not use these RDD best practices for the 1957 cohort MLSA rise with UK Biobank data.
Instead, use of a hard-coded bandwidth of 10 years and a hardcoded polynomial order 1 or 2.
RDD estimates using this specification show highly efficient estimates of the education returns in the UKB sibling sample --- results which do not survive RDD best practices.
...
These alternative approaches require structure on how invalid instruments violate exclusion; the most common adjustment for genetic settings, MR-egger \citep{bowden2015mendelian}, relies on the direct genetic effects being uncorrelated with instrument strength, paralleling the polarised-invalid-instruments assumption in the econometrics literature \citep{kolesar2015}. 
This is difficult to justify in the present setting, where the SNPs that most strongly predict education are plausibly also those with direct effects on earnings through cognitive skills, non-cognitive traits, health, field of study, occupational sorting, or other labour-market relevant channels.
