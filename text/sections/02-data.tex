...
\begin{table}[!htp]
    ...
    \begin{tabular}{l c c c c}
        ...
        \\[-1.8ex]\hline \\[-1.8ex]
        Observations
            & \input{sections/tables/ukb-analysis-count.txt} &
            & \input{sections/tables/ukb-total-count.txt} & \\
        \\[-1.8ex]\hline \\[-1.8ex]
    \end{tabular}
    \vspace{-0.25cm}
...
These indices are fundamentally correlational measures: they capture statistical associations between genetic variants and outcomes observed in large samples, not well understood biological pathways, and without a causal interpretation.
...
\begin{figure}[!h]
    ...
    The right plot shows a bin scatter, visualisaling a (semi-parametric) correlation between the Ed PGI and education years.
...
To overcome this missing data problem, I also use siblings' genetic data to impute the mean parental PGIs from the sample of \input{sections/tables/ukb-analysis-count.txt} participants with at least one family member (parent or sibling) also in the UKB --- the analysis sample described above.
...
In IBD1, one allele is shared between the siblings, leaving one parental allele unobserved; depending on whether the shared allele is the reference allele or not, the imputed parental sum is either $1 + f$ or $2 + f$, depending on value of the inherited shared allele.
%\footnote{
