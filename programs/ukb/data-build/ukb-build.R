...
# Adjust the columns for CPI 2024.
ukb_soc.data <- ukb_soc.data %>%
    left_join(cpi.data, by = c("visityear" = "year")) %>%
    transmute(
        eid = eid,
        soc_mean_hourly       = cpi.factor * (soc_mean_hourly / (cpi2015 / 100)),
        soc_median_hourly     = cpi.factor * (soc_median_hourly / (cpi2015 / 100)),
        soc_mean_hourly_all   = cpi.factor * (soc_mean_hourly_all / (cpi2015 / 100)),
        soc_median_hourly_all = cpi.factor * (soc_median_hourly_all / (cpi2015 / 100)),
        soc_2d_hourly         = cpi.factor * (exp(log_y_hourly) / (cpi2015 / 100)))

# Do the same for the household mid-point variable, updated to 2024 CPI-H.
cleaned_pheno.data <- cleaned_pheno.data %>%
    left_join(cpi.data, by = c("visityear" = "year")) %>%
    mutate(householdincome_midpoint =
        cpi.factor * (householdincome_midpoint / (cpi2015 / 100))) %>%
    select(-cpi2015)
