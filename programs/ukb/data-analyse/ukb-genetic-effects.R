...
# Define the outcome, regressors, and controls.
...
#! Test: try the DML estimation with the control variables.
# OLS to compare against.
edyears.ols <- feols(edyears ~ 1 + edpgi_all_imputed_self + edpgi_all_imputed_parental
    + adhd_pgi + asthma_pgi + bipolar_pgi + bmi_pgi + height_pgi + schizophrenia_pgi + t2diabetes_pgi
    | visityear + sibling_count + sex_male + birthyear + urban + mother_present + father_present,
    cluster = "famid",
    data = analysis.data)
print(summary(edyears.ols))
# DML version
library(DoubleML)
library(data.table)
library(mlr3)
library(mlr3learners)
control.columns <- c("edpgi_all_imputed_parental",
    "adhd_pgi", "asthma_pgi", "bipolar_pgi", "bmi_pgi", "height_pgi", "schizophrenia_pgi", "t2diabetes_pgi",
    "visityear", "sibling_count", "sex_male", "birthyear", "urban", "mother_present", "father_present")
dml.data <- DoubleMLData$new(data.table(analysis.data),
    y_col = "edyears",
    d_cols = "edpgi_all_imputed_self",
    x_cols = control.columns)
print(dml.data)
# Define the ML learners.
lgr::get_logger("mlr3")$set_threshold("warn")
learner <- lrn("regr.ranger", num.trees = 10^3,
    mtry = length(control.columns), max.depth = 10, min.node.size = 2)
#! Note" mtry = size of controls, because cannot leave out parental Ed PGI
input.reg <- learner$clone()
outcome.reg <- learner$clone()
edyears.dml <- DoubleMLPLR$new(dml.data, input.reg, outcome.reg)
edyears.dml$fit()
print(edyears.dml)
# Compare DML to OLS output.
print(edyears.dml$summary())
print(summary(edyears.ols))
#TODO: Add this to the appendix table for each regression, to ameliorate OLS weightings.
...
# Save this plot
ggsave(file.path(figures.folder, "edpgi-edyears-ols.png"),
    plot = edpgi_edyears.plot,
    units = "cm", width = fig.width, height = fig.height)
    
# Show correlation between Ed PGI and edyears, with causal lines.
edpgi_edyears.plot <- analysis.data %>%
    binscatter.plot(data = .,
        "edpgi_all_imputed_self", "edyears", colour.list[2],
        option = "half-line",
...
edpgi_earnings.plot <- analysis.data %>%
    ...
    binscatter.plot(data = .,
        "edpgi_all_imputed_self", "soc_median_hourly", colour.list[3]) +
    # Annotate OLS
    ...
    theme(plot.title = element_text(size = rel(1), hjust = 0),
        ...
        plot.margin = unit(c(0.5, 3, 0, 0), "mm"))
# Save this plot
ggsave(file.path(figures.folder, "edpgi-earnings-ols.png"),
    plot = edpgi_earnings.plot,
    units = "cm", width = fig.width, height = fig.height)

# Show correlation between Ed PGI and income, with causal line.
edpgi_earnings.plot <- analysis.data %>%
    mutate(log_soc_median_hourly = log(soc_median_hourly)) %>%
