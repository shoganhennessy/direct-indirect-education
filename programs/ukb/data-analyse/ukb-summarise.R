...
## Load packages, in the RENV environment
renv::load()
...
analysis.data %>%
    ...
    print(width = 1000)
# Get a fictitious person to describe in here (off-set from a real person).


################################################################################
## Get observation data on family definitions.

# Load the relative connections data
ukb_relatives.data <- file.path("..", "..", "..", "data", "ukb-restricted", "input") %>%
    file.path("kinship-adjusted.dat") %>%
    read_tsv()

# Take out non-respondents.
rows_remove.data <- file.path("..", "..", "..", "data", "ukb-restricted", "input") %>%
    file.path("rows-remove.csv") %>%
    read_csv()

# Load the phenotype data (raw format from UKB RAP servers).
ukb_relatives.data <- ukb_relatives.data %>%
    # Extract withdrawn rows
    filter(!(ID1 %in% rows_remove.data$eid)) %>%
    filter(!(ID2 %in% rows_remove.data$eid))

# Get fam definitions.
relatedness.definitions <- tribble(
    ~InfType,  ~relationship,                          ~kinship_coefficient, ~IBS0,
    "Dup/MZ",  "Duplicate / Monozygotic twins",         "$> 0.3540$",         "",
    "PO",      "1st degree / Parent-child",             "$0.1770$--$0.3540$", "$< 0.0012$",
    "FS",      "1st degree siblings",                   "$0.1770$--$0.3540$", "$\\geq 0.0012$",
    "2nd",     "2nd--3rd degree relatives / cousins",   "$0.0442$--$0.1770$", ""
)

# Count pair types in calculated UKB relatedness file
relatedness.table <- ukb_relatives.data %>%
    count(InfType, name = "N_pairs") %>%
    right_join(relatedness.definitions, by = "InfType") %>%
    mutate(N_pairs = replace_na(N_pairs, 0L),
        InfType = factor(InfType, levels = c("Dup/MZ", "PO", "FS", "2nd"))) %>%
    arrange(InfType) %>%
    transmute(
        `Relationship type` = relationship,
        `Kinship coefficient` = kinship_coefficient,
        `IBS0` = IBS0,
        `N (pairs)` = N_pairs)
# Add total row
relatedness.table <- relatedness.table %>%
    bind_rows(tibble(
        `Relationship type` = r"(\hline Total)",
        `Kinship coefficient` = "",
        `IBS0` = "",
        `N (pairs)` = sum(relatedness.table$`N (pairs)`)))

# Save LaTEX table for appendix.
print(relatedness.table)
relatedness.table %>%
    xtable() %>%
    print(
        ...
        file = file.path(tables.folder, "ukb-relations-count.tex"))
...
edyears.int <- coef(summary(edyears.reg))["(Intercept)", "Estimate"]
...
edyears_earnings.plot <- analysis.data %>%
    filter(soc_median_annual > 0) %>%
    mutate(predicted_income = 5 + exp(predict(edyears.reg))) %>%
    group_by(edyears) %>%
    summarise(
        soc_median_annual = mean(soc_median_annual, na.rm = TRUE),
        predicted_income = mean(predicted_income, na.rm = TRUE)) %>%
    ungroup() %>%
    ggplot(aes(x = edyears)) +
    geom_smooth(aes(y = predicted_income), method = "loess", se = FALSE,
        colour = colour.list[2], fill = colour.list[2], size = 2, alpha = 0.9) +
    geom_point(aes(y = soc_median_annual), colour = "black", size = 2) +
