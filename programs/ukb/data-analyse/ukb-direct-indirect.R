...
table.entries <- data.frame(value = c(
    r"(Education effect, $\pi$)", "",
    r"(Total genetic effect, $\theta$)", "",
    r"(Education returns, $\beta$)", "",
    r"(\midrule Direct, Ed PGI effect $\theta - \pi \beta$)", "",
    r"(Indirect, education effect $\pi \beta$)", "",
    r"(Proportion mediated through education, $\frac{\pi \beta}{\theta}$)", "",
    "Observation count"))

# Loop across the outcomes.
for (outcome.entry in outcome.list){
    print(outcome.entry)
    # Estimate on the outcome (without controls).
...
outcome.labels <- c("Occupational wage", "Occupational income", "Household income")
table.data <- data.frame(
    outcome = rep(outcome.labels, 4),
    effect = c(rep("Total", 3), rep("Direct", 3), rep("Indirect", 3),
        rep("Proportion Education Channel", 3)),
    point.est = as.numeric(c(
        table.entries[3, c(3, 5, 7)],
        table.entries[7, c(3, 5, 7)],
        table.entries[9, c(3, 5, 7)],
        table.entries[11, c(3, 5, 7)])),
    point.se = as.numeric(str_sub(c(
        table.entries[4, c(3, 5, 7)],
        table.entries[8, c(3, 5, 7)],
        table.entries[10, c(3, 5, 7)],
        table.entries[12, c(3, 5, 7)]), 2, -2))) %>%
    mutate(
        point.lower = point.est - 1.96 * point.se,
        point.upper = point.est + 1.96 * point.se)
print(table.data)

# Plot health results in a bar chart.
table.plot <- table.data %>%
    filter(effect != "Proportion Education Channel") %>%
    ggplot(aes(
        fill = factor(effect, levels = c("Total", "Direct", "Indirect")),
        x = outcome)) +
    geom_bar(aes(y = point.est),
        stat = "identity", position = "dodge", colour = "black") +
    geom_errorbar(aes(ymin = point.lower, ymax = point.upper),
        size = 2 / 3,
    ...
    scale_x_discrete(
        name = "",
        limits = outcome.labels) +
    scale_y_continuous(expand = c(0, 0, 0.01, 0),
        limits = c(-0.001, 0.08), breaks = seq(-0.1, 0.1, by = 0.01),
        oob = scales::rescale_none,
        name = "") +
    ggtitle("Estimate, percent Ed PGI effect on income outcomes") +
...
# Save the plot.
ggsave(file.path(figures.folder, "correlational-mediate.png"),
    plot = table.plot,
    units = "cm", dpi = 300, width = 2 * fig.width, height = fig.height)

# Plot health results in a bar chart.
percent.plot <- table.data %>%
    filter(effect == "Proportion Education Channel") %>%
    ggplot(aes(x = outcome)) +
    geom_bar(aes(y = point.est), fill = "orange",
        stat = "identity", position = "dodge", colour = "black") +
    geom_errorbar(aes(ymin = point.lower, ymax = point.upper),
        size = 2 / 3,
        stat = "identity", position = position_dodge(0.9), width = 1 / 3) +
    ...
    scale_x_discrete(
        ...
        limits = outcome.labels) +
    scale_y_continuous(expand = c(0, 0, 0.01, 0),
        limits = c(-0.01, 1.01), breaks = seq(0, 1, by = 0.1),
        oob = scales::rescale_none,
        name = "") +
    ggtitle("Estimate, Proportion mediated through education") +
    theme(plot.title = element_text(size = rel(1), hjust = 0),
        plot.title.position = "plot",
        plot.margin = unit(c(0, 3, 0.25, 0), "mm"),
        legend.position = c(0.8, 1.025),
...
# Save the plot.
ggsave(file.path(figures.folder, "correlational-mediate-percent.png"),
    plot = percent.plot,
    units = "cm", dpi = 300, width = 2 * fig.width, height = fig.height)
...
mediate.sensitivity.boot <- function(beta.entry, outcome.entry, given.data,
    cluster.var, R = 10^3) {
    ...
    # Boot-compatible wrapper that calls existing function (with clusters)
    cluster.ids <- unique(given.data[[cluster.var]])
    cluster.map <- split(seq_len(nrow(given.data)), given.data[[cluster.var]])
    boot.wrapper <- function(data, indices) {
        boot.data <- given.data[unlist(cluster.map[data[indices]]), ]
        mediate.sensitivity(beta.entry, outcome.entry, boot.data,
            indices = 1:nrow(boot.data), boot = FALSE)
    }
    # Run the bootstrap
    boot.out <- boot(data = cluster.ids, statistic = boot.wrapper, R = R)
...
sensitivity.data <- do.call(rbind, lapply(beta.list, function(beta.entry) {
    print(beta.entry)
    mediate.sensitivity.boot(beta.entry, outcome.list[1], analysis.data,
        "famid", R = 10^3)
}))
...
direct.plot <- sensitivity.data %>%
    ...
    # COrrelational ed returns estimate
    geom_vline(xintercept = 100 * ed.returns.point,
        linetype = "dashed", alpha = 0.99, colour = "orange") +
    annotate("text", x = (100 * ed.returns.point - 1.85), y = 0.1,
        label = "Correlational \n Estimate",
        fontface = "bold", colour = "orange") +
    # 1. Direct effect
    ...
    annotate("text", colour = colour.list[1],
        x = 12.5, y = -0.05, fontface = "bold", label = "Direct Genetic Effect") +
    # 2. Indirect effect
    ...
    annotate("text", colour = colour.list[2],
        x = 12.5, y = 0.1, fontface = "bold", label = "Indirect Education Effect") +
    # 3. Total effect
    ...
    annotate("text", colour = colour.list[3],
        x = 12.5, y = 0.025, fontface = "bold", label = "Total Effect") +
    theme_bw() +
    scale_x_continuous(expand = c(0, 0),
        name = "Education Return Value, Percent",
        limits = 100.5 * c(min(beta.list), max(beta.list)),
        breaks = 100 * seq(min(beta.list), max(beta.list), by = 0.025)) +
...
income_sensitivity.data <- do.call(rbind, lapply(beta.list, function(beta.entry) {
    print(beta.entry)
    mediate.sensitivity.boot(beta.entry, outcome.list[2], analysis.data,
        "famid", R = 10^3)
}))
# Operate the sensitivity analysis for the third
house_sensitivity.data <- do.call(rbind, lapply(beta.list, function(beta.entry) {
    print(beta.entry)
    mediate.sensitivity.boot(beta.entry, outcome.list[3], analysis.data,
        "famid", R = 10^3)
}))

# Get each returns to education.
ed.returns.list <- c()
for (outcome.entry in outcome.list){
    edreturns.reg <- lm(formula(paste0(
        outcome.entry, mediator.entry, control.list)), data = analysis.data)
    ed.returns.point <- avg_slopes(edreturns.reg, hypothesis = "edyears = 0")$estimate
    ed.returns.list <- c(ed.returns.point, ed.returns.list)
}

# Combine these data, to plot.
sensitivity_combined.data <- rbind(
    mutate(wage_sensitivity.data, edreturns = ed.returns.list[1],
        outcome = "Occ hourly wage"),
    mutate(income_sensitivity.data, edreturns = ed.returns.list[2],
        outcome = "Occ annual income"),
    mutate(house_sensitivity.data, edreturns = ed.returns.list[3],
        outcome = "Occ household income (midpoint)"))
...
percent.plot <- sensitivity_combined.data %>%
    ...
    geom_vline(xintercept = 100 * ed.returns.point,
        linetype = "dashed", alpha = 0.99, colour = "orange") +
    annotate("text", x = 2.5, y = 1.25,
        label = "Correlational \n Estimate",
        size = 2.5,
        fontface = "bold", colour = "orange") +
    #geom_vline(aes(xintercept = 100 * mean(edreturns)),
    #    linetype = "dashed", alpha = 0.99) +
    geom_line(aes(y = indirect.share), colour = colour.list[2], linewidth = 1) +
    geom_ribbon(alpha = 0.2, fill = colour.list[2], aes(
...
ggsave(file.path(figures.folder, "sens-percent.png"),
    plot = percent.plot,
    units = "cm", dpi = 300,
    width = 2.5 * fig.width, height = fig.height)
...
boot_table.data <- data.frame(value = c(
    r"(\textbf{Education and Total Effect Estimates:} \\ Total genetic effect, $\theta$)", "",
    r"(Education returns, $\beta$)", "",
    r"(\midrule \textbf{Mediation Estimates:} \\ Direct, Ed PGI effect $\theta - \pi \beta$)", "",
    "r(Indirect, education effect $\pi \beta$)", "",
    "r(Proportion mediated through education, $\frac{\pi \beta}{\theta}$)", "",
