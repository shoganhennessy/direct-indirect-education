...
# First-stage OLS estimate of π_1 (this model is identified).
firststageEstimate = IV2SLS.from_formula("X ~ 1 + Z_1", data=simData)
firststageEstimate = firststageEstimate.fit()
firststageEstimate = firststageEstimate._params["Z_1"]
...
π_1Hat = IV2SLS.from_formula("X ~ 1 + Z_1", data=simData).fit()._params["Z_1"]
α_1Hat = IV2SLS.from_formula("Y ~ 1 + Z_1", data=simData).fit()._params["Z_1"]
...
print("β value and naive OLS estimate", [β, olsEstimate])
print("γ_1 =", γ_1,
...
π_2Hat = IV2SLS.from_formula("X ~ 1 + Z_2", data=simData).fit()._params["Z_2"]
α_2Hat = IV2SLS.from_formula("Y ~ 1 + Z_2", data=simData).fit()._params["Z_2"]
...
## Save the figure
#plt.show()
plt.savefig("../../paper/figures/two-dimension.png",
    dpi=300, transparent=False, bbox_inches="tight")


#! Conclusion:
# This gives a way to bound estimates for β, by using information from two
# instruments, which have effects in different directions.
# The parameters (π_1, π_2, γ_1, γ_2) characterise the bounded system, and
# exactly one (of the four) must have a different sign to the others for the
# bounds to be valid.
# This gives a way to estimate β even if exclusion restriction is very wrong.
#TODO: Lingering questions
# Each instrument estimates a LATE, so the bounds are only valid if the 
# respective complier groups are the same/comparable.  Unsure if verifiable.
#Next -> can use the Bayesian priors for gamma_1, gamma_2 to point identify beta.
# This can be super useful for the IGE estimate bounds.
